...
inputaudiomg.innerHTML = '<div class="container_field_import"><label for="file_audio_mg">contenu pour les nationaux</label><div class="boutton_file">choisir</div><input id="file_audio_mg" class="file_book" name="contenu_mg[]" type="file" accept="audio/* , application/vnd.openxmlformats-officedocument.wordprocessingml.document , application/msword , application/pdf" multiple /><span class="file_name">Aucune fichier selectionné</span></div>';

let inputaudiofr = document.createElement("div");
inputaudiofr.classList = 'form-group';
inputaudiofr.innerHTML = '<div class="container_field_import"> <label for="file_audio_et">contenu pour les etrangers</label><div class="boutton_file">choisir</div><input id="file_audio_et" class="file_book" name="contenu_fr[]" type="file" accept="audio/* , application/vnd.openxmlformats-officedocument.wordprocessingml.document , application/msword , application/pdf" multiple /><span class="file_name">Aucune fichier selectionné</span></div>'
...
selectformat.addEventListener("change", function (event) {
    if (event.target.value == 2) {
        ...
        bottonfile = document.querySelectorAll(".boutton_file");
        inputfile = document.querySelectorAll(".file_book");
        filename = document.querySelectorAll(".file_name");
        file = new InputFile(inputfile, bottonfile, filename);
        file.attachToBottonInputFile(bottonfile, inputfile, filename);
